...
function StackedAreaChart(data) {
    ...
    var element = document.getElementById('alertSammary-Chart'),
        height = 310;

    // Remove old chart to load new one 
    d3.select(element).selectAll("svg").remove();
    d3.select(element).selectAll(".d3-tip e").remove();
    ...
    if (element) {
        if (typeof (data) !== 'undefined' && data.length > 0) {
            ...
            var d3Container = d3.select(element),
                margin = { top: 5, right: 15, bottom: 40, left: 15 },
            ...
            // Colors   
            var colorrange = ['#225ea8', '#1e90c0', '#40b6c4', '#7fcdbb', '#c7e8b4', '#edf8b1', '#ffe2aa', '#4fc3f7', '#ffb74d', '#81c784', '#81D4FA', '#494c79', '#515483', '#5e6398', '#DD2115'];
            ...
            var tickFormatDate, ticks, ticksNo;
            switch (period) {
                case 1:
                case 2:
                case 3:
                    tickFormatDate = d3.time.format(userTimeFormat); // Display hours and minutes in 24h format
                    ticks = d3.time.hours;
                    ticksNo = 2;
                    break;
            }
            ...
            var group = svg.append('g')
                .attr('class', '.stackedarea-leyer-group');

            if (layers !== undefined) {
                // And append paths to this group
                var layer = group.selectAll(".stackedarea-leyer")
                    .data(layers)
                    .enter()
                    .append("path")
                    .attr("class", "stackedarea-leyer d3-slice-border")
                ...
                // Left vertical
                svg.append("g")
                    .attr("class", "d3-axis d3-axis-left hidden")
                ...
                xaxisg.selectAll(".d3-axis-subticks")
                    .data(x.ticks(ticks), function (d) { return d; })
                    .enter()
                    .append("line")
                ...
                layerTransition.each("end", function () {
                    layer
                        .on("mouseover", function (d, i) {
                            svg.selectAll(".stackedarea-leyer")
                                .transition()
                                .duration(250)
                        ...
                        .on("mousemove", function (d, i) {
                            ...
                            tooltip.html(
                                ...
                                '<li>' + '<div class="font-size-base my-1"><i class="fa fa-chevron-circle-left"></i><span class="d-inline-block ml-2"></span>' + d.key + '</div>' + '</li>' +
                                '<li>' + 'Triggered: &nbsp;' + "<span class='font-weight-semibold f-right'>" + pro + '</span>' + '</li>' +
                                '<li>' + 'Time: &nbsp; ' + '<span class="font-weight-semibold f-right">' + tickFormatDate(d.values[mousedate].date) + '</span>' + '</li>' +
                                '</ul>'
                            )
                                .style("display", "block");
            ...
            d3Container
                .on("mousemove", function (d, i) {
                    ...
                    if (mousex >= ($(element).outerWidth() - $('.d3-tip').outerWidth() - margin.right - (tooltipOffset * 4))) {
                        tooltip
                            .style("left", (mousex - $('.d3-tip').outerWidth() - tooltipOffset * 2) + "px") // Change tooltip direction from right to left to keep it inside graph area
                            .attr("class", "d3-tip w");
                    }
                    else {
                        tooltip
                            .style("left", (mousex + tooltipOffset * 2) + "px")
                            .attr("class", "d3-tip e");
                    }                    
                });
            ...
            function resizeStream() {
                ...
                // Area paths
                svg.selectAll('.stackedarea-leyer').attr("d", function (d) { return area(d.values); });
            }
        }
    }
