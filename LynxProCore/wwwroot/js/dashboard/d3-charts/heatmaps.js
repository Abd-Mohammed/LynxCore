/* ------------------------------------------------------------------------------
 *  # D3.js v3 - Heatmaps chart
 * ---------------------------------------------------------------------------- */

function HeatmapsChart(element, data) {
    ...
    if ($(element).length > 0) {
        ...
        if (d3.select(element).selectAll("svg").size() > 0 && typeof (data) !== 'undefined') {
        ...
        if (typeof (data) !== 'undefined' && data.length > 0) {
            ...
            // ------------------------------                
            // Nest data
            var nested_data = d3.nest().key(function (d) { return d.areaName; }),
                nest = nested_data.entries(data);

            // Format date
            var format = d3.time.format("%Y-%m-%dT%H:%M:%S"),
                period = parseInt($('#Period').val()),
                formatTime, tickFormat, gridFormat;
            ...
            switch (period) {
                case 1:
                    formatTime = d3.time.format("%H:%M"); // Display hours and minutes in 24h format   
                    tickFormat = d3.time.hour.offset(new Date(data[data.length - 1].date), 1);
                    gridFormat = new Date(data[data.length - 1].date).getHours();
                    break;
                case 7:
                    formatTime = d3.time.format("%H:%M %a"); // Display days name in 1 week format
                    tickFormat = d3.time.day.offset(new Date(data[data.length - 1].date), 1);
                    gridFormat = new Date(data[data.length - 1].date).getDay() + new Date(data[data.length - 1].date).getHours();
                    break;
                case 30:
                    formatTime = d3.time.format("%b %d"); // Display day with month in 1 month format
                    tickFormat = d3.time.week.offset(new Date(data[data.length - 1].date), 1);
                    gridFormat = new Date(data[data.length - 1].date).getDay() + new Date(data[data.length - 1].date).getHours();
                    break;
            }
            ...
            margin = { top: 20, right: 0, bottom: 30, left: 0 },
                ...
                gridSize = width / gridFormat, // dynamically set grid size 
                ...
                colors = ['#e9d8fa', '#D2B4DE', '#b990e1', '#9f6bd3', '#8548c3'];
            ...
            // Horizontal
            x.domain([new Date(data[0].date), tickFormat]);

            // Vertical
            y.domain([0, d3.max(data, function (d) { return d.areaName; })]);
            ...
            hourGroup
                ...
                .text(function (d, i) { return d3.sum(d.values, function (d) { return d.value; }) + ' trips' });
            ...
            hourGroup.each(function (d) {
                heatMap
                    .on('mouseover', function (d, i) {
                        ...
                        d3.select(this.parentNode).select('.sales-count').text(function (d) { return d.values[i].value + ' trips at ' + formatTime(d.values[i].date); })
                    })
                    .on('mouseout', function (d, i) {
                        d3.select(this).style('opacity', 1);
                        d3.select(this.parentNode).select('.sales-count').text(function (d, i) { return d3.sum(d.values, function (d) { return d.value; }) + ' trips' })
                    })
            })
