...
mohammad.kamal@acacusgroup.com
...
ContextMenu.prototype.onAdd = function () {
    function createMenuItem(values) {
        ...
        if (values.image) {
            menuItem.style.backgroundImage = 'url(' + values.image + ')';
        }

        menuItem.onclick = function () {
            $this.hide();
            google.maps.event.trigger($this, 'menuitemselected', $this.position_, values.eventName);
        };
...
ContextMenu.prototype.show = function (latLng, vehicleId, plateNo, serialNo, vehicleName, driverId, streamingChannelCount, ipCameraChannelNo, deviceCode) {
    if (!this.isVisible_) {
    ...
    this.plateNo_ = plateNo;
    this.serialNo_ = serialNo;
    this.streamingChannelCount_ = streamingChannelCount;
    this.ipCameraChannelNo_ = ipCameraChannelNo;
    this.vehicleName_ = vehicleName;
    this.driverId_ = driverId;
...
//TODO
//SurveillanceMap
ContextMenu.prototype.showVehicleMenu = function (tenantId, tenantName, latLng, vehicleId, plateNo, serialNo, vehicleName, streamingChannelCount, ipCameraChannelNo, deviceCode) {
    if (!this.isVisible_) {
        this.menu_.style.display = 'block';
        this.isVisible_ = true;
    }
...
ContextMenu.prototype.showTrackedItemMenu = function (tenantId, latLng, trackedItemId, trackedItemName) {
    if (!this.isVisible_) {
        this.menu_.style.display = 'block';
        this.isVisible_ = true;
    }
    ...
    this.position_ = latLng;
    this.tenantId_ = tenantId;
    this.trackedItemId_ = trackedItemId;
    this.trackedItemName_ = trackedItemName;
